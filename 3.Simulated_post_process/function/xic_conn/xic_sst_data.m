...
function [go_wrong, stop_suces, stop_failure, sub_sst_subject] = xic_sst_data(files)

go_wrong = zeros(268,268,length(files));
stop_suces = zeros(268,268,length(files));
stop_failure = zeros(268,268,length(files));
...
try
    ...
    go_wrong_indi=data.Z(:,1:268);
    go_wrong(:,:,j) = go_wrong_indi;  

    subject = files(j).name;
    subject =str2double(subject(9:end));
...
if sum(stop_failure_number)~=0
   ...
   data = load(fullfile(files_mat(number).folder,files_mat(number).name));
   stop_failure_indi=data.Z(:,1:268);
