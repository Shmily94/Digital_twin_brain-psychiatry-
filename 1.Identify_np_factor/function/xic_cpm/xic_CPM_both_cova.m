...
function [CPM_Result] = xic_CPM_both_cova(all_mats,mat_sub,all_behav,beha_sub,thresh,cova,cova_sub)
...
all_mats= all_mats';
 ...
 [all,b] = intersect(beha_sub,mat_sub);
 [all,b] = intersect(all,cova_sub);
    
    
    [al,ph] = intersect(beha_sub,all);
    [al,cova_s] = intersect(cova_sub,all);
    [al,eft] = intersect(mat_sub,all);
    
   
    all_behav = all_behav(ph);
    cova_data = cova(cova_s,:);
    all_mats = all_mats(:,:,eft);
    subject_all =al;
    
   
    all_mats = reshape(all_mats,[],size(all_mats,3))';
    nan_mask = all_mats(1,:);
    all_mats(:,isnan(nan_mask))=[];
    
    all_mats = xic_cpm_regress(all_mats,cova_data);
    all_behav = xic_cpm_regress(all_behav,cova_data(:,1:8));    
...
[CPM_mask] = xic_CPM_mask(all_mats,mask_pos,mask_both,nan_mask);


end


function  [mask_cpm] = xic_CPM_mask(all_mats,mask_pos,mask_both,nan_mask)
...
all_mats= all_mats';
    ...
    all_mats_nan_mask = nan_mask;
   
    
    % ----------------------
  
    size2 = sqrt(length(nan_mask));
    ...
    mask_pos_pro = sum(mask_pos_se,2)/size(all_mats,1);
...
%     mask_neg_pro = round(reshape(mask_neg_pro,size2,size2));
    ...
    mask_both_pro = sum(mask_both_se,2)/size(all_mats,1);
