...
function [CPM_Result] = xic_CPM_both_square(all_mats,all_behav,thresh)
...
%     end
    ...
    pos_mats = train_mats(:,pos_mask==1);
    neg_mats = train_mats(:,neg_mask==1);
    
    pos_corr = corr(pos_mats);neg_corr = corr(neg_mats);
    pos_corr = pos_corr.^2;neg_corr = neg_corr.^2;
    pos_sum = sum(pos_corr,2)-1; neg_sum = sum(neg_corr,2)-1;
    
    
    pos_r = r_mat(pos_mask==1);neg_r = r_mat(neg_mask==1);
    pos_r_weight = pos_r./pos_sum;  pos = repmat(pos_r_weight,1,size(pos_mats,1))';
    neg_r_weight = neg_r./neg_sum;  neg = repmat(neg_r_weight,1,size(neg_mats,1))';
    
    train_sumpos = sum((pos_mats.*pos),2);
    train_sumneg = sum((neg_mats.*neg),2);
    
%     train_sumpos = sum(train_mats(:,pos_mask==1),2);
%     train_sumneg = sum(train_mats(:,neg_mask==1),2);
% %     
%     
    ...
    fit_regree = regress(train_behav,[ones(size(train_sumpos,1),1),train_sumpos,train_sumneg]);
    % run model on TEST sub

    test_mat = all_mats(leftout,:);
    
    pos_test = test_mat(:,pos_mask==1); pos = repmat(pos_r_weight,1,size(pos_test,1))';
    neg_test = test_mat(:,neg_mask==1); neg = repmat(neg_r_weight,1,size(neg_test,1))';
    
    test_sumpos = sum((pos_test.*pos),2);
    test_sumneg = sum((neg_test.*neg),2);
       
%     
%     test_sumpos = sum((test_mat((pos_mask==1))))/2;
%     test_sumneg = sum((test_mat((neg_mask==1))))/2;
    
    behav_pred_pos(leftout,1) = fit_pos(1)*test_sumpos + fit_pos(2);
    behav_pred_neg(leftout,1) = fit_neg(1)*test_sumneg + fit_neg(2);
    behav_pred_both(leftout,1) = fit_both(1)*(test_sumpos-test_sumneg) + fit_both(2);
    behav_pred_regress(leftout,1) = fit_regree(2)*test_sumpos + fit_regree(3)*test_sumneg;
    toc
end
...
test4 = all_behav;
test4((isnan(behav_pred_regress)))=[];
behav_pred_regress(isnan(behav_pred_regress))=[];
...
[R_regres, P_regress] = corr(behav_pred_regress,test4);
...
CPM_Result.R_regres_ = R_regres_;
CPM_Result.P_regress = P_regress;
CPM_Result.pos_predict = behav_pred_regress;
CPM_Result.pos_test = test4;
