...
function [CPM_Result] = xic_CPM(all_mats_set,all_behav_set,thresh_set)
...
mask_pos_final = zeros(size(all_mats,2),length(no_sub));
mask_neg_final = zeros(size(all_mats,2),length(no_sub));
    
beta_pos  = zeros(no_sub,1);
beta_neg  = zeros(no_sub,1);
parfor leftout = 1:no_sub
    ...
    beta_pos(leftout) = fit_pos(2);
    beta_neg(leftout) = fit_neg(2);
