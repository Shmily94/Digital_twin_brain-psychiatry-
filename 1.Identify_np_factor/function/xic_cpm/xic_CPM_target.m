...
function [CPM_Result] = xic_CPM_target(all_mats,all_behav,thresh,target_behav)
...
parfor leftout = 1:no_sub
    ...
    train_behav = all_behav;  train_target = all_target;
    train_behav(leftout) = [];
    train_target(leftout) = [];
    % correlate all edges with behavior
    [r_mat_target,p_mat_target] = corr(train_mats,train_target);
    r_mat_target(isnan(r_mat_target))= 0;
    p_mat_target(isnan(p_mat_target))= 0; 
    
    pos_edges_target = find(r_mat_target > 0 & p_mat_target < thresh);
    neg_edges_target = find(r_mat_target < 0 & p_mat_target < thresh);
    
    
    train_mats_new = train_mats(pos_edges_target+neg_edges_target>0,:);  
    
    [r_mat,p_mat] = corr(train_mats_new,train_behav);
    r_mat(isnan(r_mat))= 0;
    p_mat(isnan(p_mat))= 0;
    
    % set threshold and define masks
    
    pos_mask = zeros(size(train_mats_new,2),1);
    neg_mask = zeros(size(train_mats_new,2),1);
...
%         prect_data = train_mats_new(ss,:);
%         train_sumpos(ss) = sum((prect_data((pos_mask==1))))/2;
%         train_sumneg(ss) = sum((prect_data((neg_mask==1))))/2;
%     end
    
    
    train_sumpos = sum(train_mats_new(pos_mask==1,:));
    train_sumneg = sum(train_mats_new(neg_mask==1,:));
    ...
    test_mat = all_mats(leftout,:); test_mat = test_mat(pos_edges_target+neg_edges_target>0,:); 
