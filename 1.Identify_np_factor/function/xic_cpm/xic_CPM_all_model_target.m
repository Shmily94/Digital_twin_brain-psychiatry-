...
function [CPM_Result] = xic_CPM_all_model_target(all_mats_toghther,beha_cpm,all_behav,thresh)
...
parfor leftout = 1:no_sub
    ...
    [pos_edges_mat_sum] = cellfun(@(x) zscore(sum(x,2)),pos_edges_mat_final,'UniformOutput', false); 
    [neg_edges_mat_sum] = cellfun(@(x) zscore(sum(x,2)),neg_edges_mat_final,'UniformOutput', false); 
...
%    clear test_sumpos test_sumneg
    ...
    pos_test_sumpos_sum = zscore(sum(cell2mat(test_sumpos_sum),2));
    neg_test_sumneg_sum = zscore(sum(cell2mat(test_sumneg_sum),2));
