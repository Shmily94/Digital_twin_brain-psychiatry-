...
function xic_CPM_Auto_test(phenotype,phenotype_name,phenotype_subject,thresh)
...
% CPM_Result = xic_CPM_both_ten(all_mats,all_behav,thresh);
...
% 
% R_pos_all(1) = CPM_Result.R_pos;
% % sst failure
% all_mats  = all_stop_failure;
% CPM_Result = xic_CPM_both_ten_cv(all_mats,all_behav,thresh);
% CPM_Result.subject = a;
% % [CPM_mask] = xic_CPM_mask(all_mats,CPM_Result);
% 
% out_name = [phenotype_name,'_CPM_SST_failure.mat'];
% save(out_name,'CPM_Result');
% R_pos_all(2) = CPM_Result.R_pos;
% 
% %  go wrong
...
all_angry = network_a_subject;
all_happy = network_h_subject;
all_neutral = network_n_subject;
